...
## 2026-09-01 — FAIL (APE 1.57%)
- Predicted 258.91 vs actual 254.92 (prior 259.77); dir hit: True; beat baseline: True; closest analyst: news.
**What went wrong:** Blend landed 258.91 vs actual 254.92 — correct direction, magnitude off by 4pts because contrarian+macro (both 261.5, betting on an RSI-38 oversold bounce) and a lukewarm technical pulled the mean up. **Root cause:** The news analyst had ground-truth — AMZN was *already* trading -1.9% mid-session — but its 0.30 weight let two counterfactual bounce-bets (0.14+0.12=0.26 combined, plus 0.24 technical) dilute a signal that was effectively observed, not predicted. We treated a live-price observation as just another opinion. **One change:** Add a rule that when news anchors to a real-time intraday quote already >1.5% off prior close, boost its weight to >=0.45 and discount any analyst predicting reversal against that observed move.
...
## 2026-09-15 — FAIL (APE 1.62%)
- Predicted 252.45 vs actual 248.42 (prior 253.54); dir hit: True; beat baseline: True; closest analyst: momentum.
**What went wrong:** Direction was right (down) but we badly undersized the drop — predicted -1.09, actual -5.12, a 1.62% APE. The bearish analysts (momentum, news both at 251.20) were themselves too timid, and contrarian/macro's up-calls (255+) pulled the blend even higher. **Root cause:** we treated a 40.85 RSI as a two-sided coin and let 26% of weight bet on a bounce that had no catalyst against an accelerating downtrend with a confirmed -0.70% premarket. **The one change:** in confirmed accelerating downtrends with negative premarket, suppress reversion analysts and let the momentum/news consensus set the magnitude — and lean it slightly *beyond* their point estimate since trend days overshoot, not undershoot.

## 2026-09-17 — FAIL (APE 2.42%)
- Predicted 245.1 vs actual 251.19 (prior 245.96); dir hit: False; beat baseline: False; closest analyst: news.
**What went wrong:** the blend printed a down-day (245.1) into a +2.1% up-day, a directional and magnitude miss. Three trend-followers (momentum/macro/technical) all clustered ~243.5-244.2 and outvoted the two correct up-callers, dragging the consensus below prior_close. **Root cause:** the ensemble over-weighted momentum-continuation signals in a textbook oversold-bounce setup — RSI 36, 5-day losing streak, AND green futures were all present, the exact conditions where continuation fails. The scorecards already flagged this: macro/technical/momentum have the worst hit rates yet still moved the blend. **The one change:** implement a regime gate — when RSI<40 and index futures are positive, invert or halve the weight on the trend-continuation trio and let news/contrarian drive the call; had news (0.2246) plus contrarian led, we'd have printed ~248 and cut APE to ~1.3%.

## 2026-09-21 — FAIL (APE 1.28%)
- Predicted 255.15 vs actual 258.45 (prior 253.71); dir hit: True; beat baseline: True; closest analyst: news.
**What went wrong:** Directionally correct but we captured only ~30% of the actual move; the +0.57% blend was smothered by macro/technical bearishness anchored to a stale RSI-36 mean-reversion read. **Root cause:** four of five analysts recited the same oversold/below-SMA technicals as a REASON, so they weren't independent votes — they were one correlated bearish signal diluting the single analyst (news) who saw the +1% futures/AI-rally catalyst. Weighting treated them as diverse when they were redundant. **One change:** when news flags a quantified index-level tailwind (futures >0.75%), cap the combined weight of RSI-driven mean-reversion analysts at ~0.15 and let news drive; test whether this improves magnitude capture on catalyst days without hurting quiet days.

## 2026-09-22 — FAIL (APE 1.74%)
- Predicted 259.42 vs actual 254.98 (prior 258.45); dir hit: False; beat baseline: False; closest analyst: contrarian.
**What went wrong:** The ensemble was directionally wrong AND lost to baseline because every analyst except contrarian echoed identical bullish inputs (1d +1.87%, above-SMA, flat premarket). That's not five independent signals — it's one signal counted five times, so the blend had zero real diversity to protect it. **Root cause:** Momentum-flavored analysts (momentum/macro/technical) treated a two-day recovery bounce as a trend, mistaking exhaustion for accumulation; RSI at 48.5 was neutral, not bullish, yet was cited as 'room to run' by three of them. Contrarian correctly flagged 'mild exhaustion near SMA resistance' but hedged to only -0.25%. **The one change:** Add a correlation-penalty — when analyst rationales share the same 2-3 core facts, collapse their effective weight and let the highest-conviction dissenter set the tilt; here that would have pulled the blend below prior_close and salvaged direction.

## 2026-09-23 — FAIL (APE 1.44%)
- Predicted 252.85 vs actual 249.27 (prior 254.98); dir hit: True; beat baseline: True; closest analyst: news.
**What went wrong:** Correct direction (down) but we stopped short by $3.58 — the blend was pulled up by macro (256.50) and momentum (256.20), both leaning on a stale +2.6% 5d momentum/mean-reversion story that was already invalidated by live intraday data showing AMZN down −1.6% by midday. **Root cause:** the meta-judge kept 21% weight on up-biased analysts on a day with a clear active-selloff signal from news, and even news assumed a phantom late-day recovery to 251.20. We priced a bounce that had no catalyst. **One change to try:** when the news analyst reports confirmed intraday directional selling with regulatory + analyst-downgrade context, override the blend by capping any up-biased analyst weight to near-zero AND shade the news number toward the observed intraday low rather than assuming reversion.
